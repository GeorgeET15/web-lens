...
export const ScenarioSuiteDashboard: React.FC<ScenarioSuiteDashboardProps> = ({ 
  ...
  const startedAt = (report as any).startedAt || (report as any).started_at || 0;
  const finishedAt = (report as any).finishedAt || (report as any).finished_at || 0;
  ...
  const flowName = report.flowName || (report as any).file_name || 'Unnamed Flow';
  ...
  const handleInvestigate = async (result: any) => {
    ...
    try {
      const data = await api.post('/api/ai/investigate-run', {
        result: result,
  ...
  const handleStabilityAudit = async () => {
    ...
    try {
      const data = await api.post('/api/ai/stability-audit', report.results.map(r => r.report));
      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-6 bg-zinc-950/30">
        ...
        <div className="grid gap-3">
          {(report.results || []).map((result) => {
            const runId = result.runId || (result as any).run_id || '???';
            return (
              <div
                key={runId}
              ...
              >
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-1 text-[9px] text-zinc-500">
                        ...
                        <span className="opacity-50 font-medium font-mono text-[8px]">#{runId.split('_').pop()}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className={cn(
                        ...
                        scenarioInsights[runId] && !scenarioInsights[runId].isCollapsed
                          ? "bg-indigo-500/20 text-indigo-400 border-indigo-500/30"
                          : "bg-indigo-500/10 text-indigo-400 border-indigo-500/20 hover:bg-indigo-500/20"
                      )}
                    >
                      {scenarioInsights[runId]?.isLoading ? (
                      ...
                      {scenarioInsights[runId] && !scenarioInsights[runId].isCollapsed ? 'Collapse' : 'Review Commentary'}
                    </button>
                    {!result.success && (
                ...
                {scenarioInsights[runId] && !scenarioInsights[runId].isCollapsed && (
                  <div className="mt-4 border-t border-white/5 pt-4 animate-in slide-in-from-top-2 duration-300">
                    <AIInsight
                      type="inspection"
                      content={scenarioInsights[runId].content}
                      isLoading={scenarioInsights[runId].isLoading}
                      isCollapsible={false}
